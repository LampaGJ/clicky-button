 ...
 * @property {number}  skewAngle           — deg, parallelogram skew via
 *   skewX on .btn-cell (clamped to ±18); 0 = no skew (default, byte-identical
 *   to pre-#34 output — see D3).
...
const defaultClickyConfig = {
  ...
  // Parallelogram skew (issue #34). deg, clamped to ±18 in buildVarMap
  // (Zod mirrors the clamp for authoring-time feedback, but the runtime
  // validator is typeof-only — see validateClickyConfig). 0 = no skew
  // (default, byte-identical output — see D3).
  skewAngle:          0,
  faceColor:    '#c8c0b4',
  textColor:    '#1a1a1a',
  fontSizeRatio:      0,
...
function buildHousingShadow(state, pressed) {
  ...
  const contact = state.contactIntensity > 0
    ? `, 0 ${Math.max(1, Math.round(wallH_px * 0.25 * keep))}px ${Math.max(2, Math.round(wallH_px * 0.5 * keep))}px 0 rgba(0, 0, 0, ${(state.contactIntensity / 100).toFixed(3)})`
    : '';

  return `0 ${ambientY}px ${ambientBlur}px 0 ${ambientColor}${contact}`;
}
...
function buildVarMap(state) {
  ...
  // Parallelogram skew (issue #34) — hard-clamped here (not just in the Zod
  // test mirror) since validateClickyConfig is typeof-only, same pattern as
  // frameWidth's fwMaxByWidth/fwMaxByHeight clamp above. 0 = no skew, the
  // byte-identical default path (D3) — every skew consumer site below is
  // gated on skewAngle_deg !== 0.
  const skewAngle_deg = Math.max(-18, Math.min(18, state.skewAngle || 0));
  // Housing-width pre-widen (issue #34, mandatory per both expert reviews):
  // .btn-cell's skewX projects a wider footprint than its layout box —
  // (containerHeight/2) * tan(angle) beyond each side, symmetric around
  // transform-origin: center — and .btn-housing is unskewed with its own
  // overflow:hidden, so without this the parallelogram's points get clipped.
  // Baked into the JS-computed housingW alongside frame width, not left to
  // a CSS-side calc().
  const skewWiden_px = skewAngle_deg !== 0
    ? Math.ceil(state.containerHeight * Math.abs(Math.tan(skewAngle_deg * Math.PI / 180)))
    : 0;
  ...
  // ('separate') path, so housingW's formula is byte-identical to pre-#36
  // output whenever segmented isn't in use (D3).
  ...
  const housingW = segmentCount > 1
    ? segmentCount * state.containerWidth + segmentDividersTotal_px + 2 * fw + skewWiden_px
    : state.containerWidth + 2 * fw + skewWiden_px;
  ...
  return {
    ...
    // Parallelogram skew (issue #34) — emitted only when skewAngle is
    // non-zero, so default output stays byte-identical (D3).
    ...(skewAngle_deg !== 0 ? {
      '--skew-angle': `${skewAngle_deg}deg`,
    } : {}),
    ...
    '--face-shadow-pressed': buildFaceShadow(state, true, 'px'),
...
function buildCss(state, scopeSelector) {
  ...
  const faceShadowPressedCq = buildFaceShadow(state, true, 'cqb');
...
function buildButtonWallCss(state) {
  ...
  // Parallelogram skew (issue #34) — counter-skew on the label/icon so text
  // stays upright while .btn-cell (the shared ancestor of wall+face, see
  // buildGridCss) shears the whole subtree. Emitted only when skewAngle is
  // non-zero (D3); the wall itself needs no transform of its own — it's a
  // sibling inside the same skewed .btn-cell and shears for free.
  const skewAngle_deg = Math.max(-18, Math.min(18, state.skewAngle || 0));
  const counterSkewCss = skewAngle_deg !== 0
...
function buildIconPlacementCss(state) {
  ...
  // Parallelogram skew (issue #34) — this edge-icon rule already sets its own
  // transform (translateY(-50%)), so the generic .btn-label/.btn-icon
  ...
  // directly into THIS rule's transform instead. Byte-identical to pre-#34
  // output when skewAngle is 0 (D3).
  const skewAngle_deg = Math.max(-18, Math.min(18, state.skewAngle || 0));
  const edgeIconTransform = skewAngle_deg !== 0
    ? 'translateY(-50%) skewX(calc(-1 * var(--skew-angle)))'
    : 'translateY(-50%)';
...
function buildGridCss(state) {
  ...
  // Parallelogram skew (issue #34) — skewX lives on .btn-cell, the shared
  // ancestor of .btn-wall + .btn-face (both experts' pinned composition
  // point): wall+cap shear together for free, KEYCAP_Y and every keyframe
  // stay translate-only (untouched), and overflow:hidden + transform on the
  // SAME element clip in local space before the shear, so no corner
  // poke-out on the cell itself. Emitted only when skewAngle is non-zero,
  // so default output stays byte-identical (D3).
  const skewAngle_deg = Math.max(-18, Math.min(18, state.skewAngle || 0));
  const cellSkewCss = skewAngle_deg !== 0
...
.btn-cell {
  ...
  background: transparent;${cellSkewCss}
}
...
function toggledRestStatePropsCss(indent) {
  ...
  return [
    ...
    `${i}transition: transform var(--press-duration) var(--transform-easing), box-shadow var(--press-duration) var(--shadow-ease-press), background-color var(--press-duration) var(--shadow-ease-press);`,
  ].join('\n');
}
...
.clicky-toggle .btn-face {
  transition: transform var(--duration) var(--transform-easing), box-shadow var(--duration) var(--shadow-ease-release), background-color var(--duration) var(--shadow-ease-release);
}
