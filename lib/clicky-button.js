...
function buildCss(state, scopeSelector) {
  ...
  // True beveled face edge (issue #68) — appended only when in use (D3).
...
.btn-face.icon-edge.icon-right {
  ...
  // True beveled face edge (issue #68, owner-named v1 example). Additive
  // overlay pseudo-element — must NOT touch `.btn-face`'s own box-shadow
  // (owned entirely by buildFaceShadow/the flush gate, per epic #56 §4's
  // hard constraint: "the existing recess inset-shadow stack must stay
  // owned by .btn-face itself; edge/top are additive overlays only").
  // Reuses the EXISTING frame-bevel alpha vars rather than inventing a
  // third, parallel bevel-intensity knob (flagged in the synthesis notes as
  // a judgment call). Gated on bevelStyle so default output is
  // byte-identical (D3).
  function buildFaceEdgeCss(state) {
    if (state.bevelStyle !== 'beveled') return '';
...
`;
  ...
  // A nested clip's radius must track its INSET from its parent's, or the two
  // arcs are not concentric and CROSS — a hard seam at the corner (issue #89).
  // Concentric nesting: inner_radius = outer_radius - inset.
  //   bottom: inset is always fw           ⇒ radius-bot - fw === --radius ✓
  //           (why the bottom always looked right — accidentally correct)
  //   top:    inset is max(0, fw - wallH)  ⇒ collapses to 0 at the default
  //           (wallH === fw), so the cell sits flush with the housing's top
  //           edge and its top corners must match the HOUSING's radius, not
  //           the cap's. Leaving them at --radius put the two arc centres
  //           frame-width apart vertically — exactly the seam the owner saw.
  // Floored at 0 so a deep wall (cellTop > radius-bot) can't go negative.
  const cellTopRadius = `max(0px, calc(var(--radius-bot) - ${cellTopExpr}))`;
  const cellRadiusDecl = state.radiusCorners
    ? `${cellTopRadius} ${cellTopRadius} var(--radius-br) var(--radius-bl)`
    : `${cellTopRadius} ${cellTopRadius} var(--radius) var(--radius)`;
