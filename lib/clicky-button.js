...
function buildGridCss(state) {
  ...
  // The recess is a STATIC hole — it must NOT animate (the cap presses into a
  // fixed recess; the recess doesn't move). It sits at its full-reveal top:
  // for a press that reaches flush that is 0 (full cell), hidden at rest behind
  // the opaque cap+wall and simply UNCOVERED as the key sinks. No cavity `top`
  // transition/keyframe → no channel bounce or animate-up (owner report). A
  // shallow press (never reaches flush) keeps the resting wall-h — its own
  // footprint covers the gap, so no reveal is needed there.
  const cavityStaticTop = cavityRevealsPress(state) ? CAVITY_PRESSED_TOP : 'var(--wall-h)';
...
.btn-cell::before {
  content: '';
  position: absolute;
  /* STATIC recess (does not animate). It sits at its full-reveal top —
     max(0, wall-h - press-translate), i.e. 0 for a press that reaches flush
     — hidden at rest behind the opaque cap+wall (which together cover the whole
     cell) and simply UNCOVERED as the key sinks. A shallow press that never
     reaches flush keeps the resting wall-h (its own footprint fills the gap,
     so nothing of the recess may show before then). See cavityStaticTop.
