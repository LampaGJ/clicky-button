...
/* Button wall — the moving side of the keycap. Drawn as .btn-face::after so it
   tracks every face transform (hover, click cycle, toggle) automatically.
   Mirrors the cavity wall (.btn-cell::before): a base color with the same
   horizontal dark-vignette gradient multiplied on top, driven by its own
   independent button-wall vars. */
function buildButtonWallCss() {
  return `/* Button label — wraps the face text so it can be clipped without
   clipping the .btn-face::after button wall (face itself is overflow:visible). */
.btn-label {
  display: block;
  max-width: 100%;
  overflow: hidden;
...
.btn-face::after {
  content: '';
  position: absolute;
  /* 1px inflation / top overlap avoids sub-pixel seams against the face and
     cavity, matching .btn-cell::before. */
  left: -1px;
  right: -1px;
  top: calc(100% - 1px);
  /* (wall-h + hover-lift) tall: the extra hover-lift overshoots the cell bottom
     (clipped) at rest, then pulls up flush on hover so the rising key never
     exposes a strip of cavity beneath it. */
  height: calc(var(--wall-h) + var(--hover-lift) + 1px);
...
.btn-cell::before {
  ...
  /* Cavity wall — the static inside of the housing slot, revealed above the
     ...
     moving button-side wall is drawn separately, as .btn-face::after, with the
     same gradient structure driven by its own button-wall vars.) */
...
function sharedFaceCssProps(indent) {
  ...
  return [
    ...
    `${i}word-break: normal;`,
    // Visible so the .btn-face::after button wall can extend below the face.
    // Long-label clipping is handled by .btn-label instead.
    `${i}overflow: visible;`,
    `${i}padding-top: 0;`,
    `${i}padding-bottom: 0;`,
