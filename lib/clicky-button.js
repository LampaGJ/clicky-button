...
`;
  ...
  // Small non-zero floor (half frame-width): a proud key whose top inset would
  // otherwise hit exactly 0 sits at 0 === the corner-tangency singularity, where
  // the cap's rounded corner touches the housing's and the chrome ring pinches
  // to zero right where the top edge meets the arc (issue #90). Lifting the inset
  // to fw/2 keeps a thin continuous ring at the top (key still reads proud — the
  // top band is half the side band) while clearing the pinch. Shallow/flat keys
  // (fw - wallH >= fw/2, i.e. wallH <= fw/2) are unaffected — byte-identical.
  const restingChromeAbove = 'max(calc(var(--frame-width) / 2), calc(var(--frame-width) - var(--wall-h)))';
