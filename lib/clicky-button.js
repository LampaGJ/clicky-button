 ...
 *   iconName/iconColor (v1 whitelist — see issue #29)
 */
...
function deriveFaceColorFamily(state, over = {}) {
  ...
  return {
    ...
    faceToggled:      darkenColor(state.useToggleColor ? state.toggleColor : faceColor, state.pressDarken),
...
.btn-wall {
  ...
  top: calc(max(var(--wall-h), var(--radius)) + var(--hover-lift));
  height: calc(100% - max(var(--wall-h), var(--radius)));
...
function buildVariantsCss(state) {
  ...
  for (const slug of slugs) {
    ...
    if (!hasFaceColor && !hasTextColor && !hasIconColor) continue;

    const lines = [];

    if (hasFaceColor) {
      const family = deriveFaceColorFamily(state, { faceColor: v.faceColor });
    ...
    if (hasTextColor || hasIconColor) {
      // darkenColor(variant iconColor ?? variant textColor ?? base, pressDarken)
      const iconColorBase = hasIconColor ? v.iconColor : v.textColor;
      const family = deriveFaceColorFamily(state, { iconColorBase });
...
.btn-face.icon-edge.icon-left {
  padding-inline-start: max(calc(var(--radius) * 0.5), var(--icon-clearance));
}

.btn-face.icon-edge.icon-right {
  padding-inline-end: max(calc(var(--radius) * 0.5), var(--icon-clearance));
}`;
}
