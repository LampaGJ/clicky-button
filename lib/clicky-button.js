...
function buildGridCss(state) {
  ...
  const _cavRel = 'var(--shadow-release-dur) var(--shadow-ease-release)';
  const _cavPrs = 'var(--shadow-press-dur) var(--shadow-ease-press) var(--shadow-press-delay)';
  ...
  const cavityBaseTransition = [
    cavityAnimates ? `top ${_cavRel}` : null,
    slotActive(state) ? `background-image ${_cavRel}` : null,
  ].filter(Boolean).join(', ');
...
.btn-cell:has(.clicky-toggle input:checked)::before {${cavityAnimates ? `
  ...
  transition: ${[
    cavityAnimates ? `top ${_cavPrs}` : null,
    slotActive(state) ? `background-image ${_cavPrs}` : null,
  ].filter(Boolean).join(', ')};
}` : '';
