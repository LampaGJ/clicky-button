 ...
 * @property {number}  restingChromeFloorRatio — % of frameWidth, issue #90.
...
function buildVarMap(state) {
  ...
  // Resting-chrome tangency floor (issue #90) — see the ClickyConfig
  // typedef's restingChromeFloorRatio doc for the geometric proof. Reused
  // below by H0 (so the housing reserves enough height for the floored
  // cellTop) and emitted as `--resting-chrome-floor` for buildGridCss's
  // restingChromeAbove expression.
  const restingChromeFloor_px = Math.round(fw * state.restingChromeFloorRatio / 100);
  ...
  const H0 = Math.max(fw, wallH_px + restingChromeFloor_px) + faceH_px + fw;
  ...
  return {
    ...
    '--resting-chrome-floor': `${Math.round(fw * state.restingChromeFloorRatio / 100)}px`,
...
`;
  ...
  const restingChromeAbove = 'max(var(--resting-chrome-floor), calc(var(--frame-width) - var(--wall-h)))';
