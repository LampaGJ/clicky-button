...
// Live scale-space geometry override (issue #96, slice 2). `.btn-scale` is the
// SOLE container-query container (the nested `.btn-cell` is no longer one — see
// buildCss's cqSupportsBlock), so every var here resolves against `.btn-scale`
// and the box reflows to whatever size/ratio the consumer gives it. Gated to
// the COMMON path — no skew, no segmented housing, no per-corner radius — which
// keeps its frozen-px base output (still functional; later slices extend it).
// Returns override lines (indented) or '' when gated out.
function buildGeometryCqLines(state) {
  const skew = resolveSkew(state);
  if (skew.active || state.housingLayout === 'segmented' || state.radiusCorners) return '';
  ...
  const fw = state.frameEnabled
    ...
    : 0;
  const housingW = cw + 2 * fw;
  const housingH = ch + 2 * fw;
  ...
  const lines = [
    ...
    `--container-width: calc(100cqw - 2 * var(--frame-width));`,
    `--container-height: calc(100cqh - 2 * var(--frame-width));`,
