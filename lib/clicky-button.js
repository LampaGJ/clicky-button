 ...
 *   (id prefix `element:`).
 */
// The cavity's pressed top edge: it tracks the descending cap so the dark
// recess fills the band the cap vacates past flush (issue #96). Shared by the
// held-press override (.btn-cell::before in buildGridCss) and the momentary
// click keyframe (clicky-cavity-top-cycle in buildClickFaceCss) — one source so
// the two press paths can never disagree. Floored at 0 for overshoot past flush.
const CAVITY_PRESSED_TOP = 'max(0px, calc(var(--wall-h) - var(--press-translate)))';

// Whether a press descends the cap to/PAST the cavity's top edge (press depth
// ≥ wall height). At/beyond that point the cap vacates a band the cavity must
// rise to fill, so the reveal applies. NOTE this is broader than
// resolveShadowTiming().animates (which is press depth STRICTLY > wall height):
// the default config has press-depth === wall-h, so `animates` is false yet the
// cap still reaches the cavity top and the reveal IS needed — verified in real
// WebKit. Shallow presses (press-depth < wall-h) never reach it → no reveal,
// byte-identical to pre-fix output.
function cavityRevealsPress(state) {
  const w = Math.max(1, Math.round(state.containerHeight * state.wallHRatio / 100));
  const p = Math.max(1, Math.round(state.containerHeight * state.pressDepthRatio / 100));
  return p >= w;
}
...
function buildGridCss(state) {
  ...
  // Cavity-reveal press tracking (issue #96): the cavity's top edge is pinned
  // at --wall-h (the flush point). When the cap descends PAST flush it vacates
  // a --wall-h-tall band at the top of the cell that nothing else covers (the
  // wall translates with the cap) — bare housing chrome showed through. When
  // the press actually reaches flush (animates), the cavity's top must track
  // the cap down to `max(0, wall-h - press-translate)` so the dark recess fills
  // that band. Gated on animates so shallow-press configs (cap never reaches
  // flush, its own footprint still covers the gap) stay byte-identical.
  const cavityAnimates = cavityRevealsPress(state);
  const cavityPressedTop = CAVITY_PRESSED_TOP;
  // The recess-reveal `top` must ride the CAP's transform timing/easing, NOT
  // the shadow's — the recess edge stays glued to the sinking cap edge
  // (cavity-top + cap-descent === wall-h at every instant), so it reads as the
  // cap uncovering a static recess instead of the channel bouncing on its own
  // under a mismatched (linear vs bouncy) easing. The lit glow channel keeps
  // the shadow timing (it's a light transfer, not a geometry track).
  const _cavTopRel = 'var(--duration) var(--transform-easing)';
  const _cavTopPrs = 'var(--press-duration) var(--transform-easing)';
  ...
  // Base-rule release transition: cavity top tracks back to rest on release
  // (animating configs); the lit glow channel fades back (slot-active).
  const cavityBaseTransition = [
    cavityAnimates ? `top ${_cavTopRel}` : null,
    slotActive(state) ? `background-image ${_cavGlowRel}` : null,
  ].filter(Boolean).join(', ');
  // Held-press / toggle-checked override: reveal the recess (and the lit
  // channel when glowing). Emitted whenever either behavior is active.
  const cavityPressOverride = (cavityAnimates || slotActive(state)) ? `
.btn-cell:has(.clicky-btn:active)::before,
.btn-cell:has(.clicky-toggle input:checked)::before {${cavityAnimates ? `
  top: ${cavityPressedTop};` : ''}${slotActive(state) ? `
  background-image: ${cavityGradientDecl()},
  var(--cavity-glow-pressed);` : ''}
  transition: ${[
    cavityAnimates ? `top ${_cavTopPrs}` : null,
    slotActive(state) ? `background-image ${_cavGlowPrs}` : null,
  ].filter(Boolean).join(', ')};
}` : '';
...
`;
  ...
  // matching the sides and bottom → an even fw ring. Combined with the derived
  // concentric housing radius (R + fw), the cap and housing corners share a
  // centre at every corner, so the ring is uniform all the way around with no
  // wedge (issue #90; supersedes the earlier resting-chrome-floor approach,
  // whose unequal top inset forced the top corners off-concentric).
...
.clicky-btn.clicky-press .btn-wall {
  ...
  // Momentary-click (.clicky-press) cavity pseudo handling: the recess-top
  // reveal (issue #96 — every animating config) and the lit glow channel
  // (issue #53b — slot-active only) share ONE binding, so both animate on the
  // same press cycle. Nothing emitted when the press never reaches flush and
  // there's no glow (byte-identical to pre-fix output).
  const cavityReveals = cavityRevealsPress(state);
  if (!cavityReveals && !slotActive(state)) return '';
  const anims = [
    cavityReveals ? 'clicky-cavity-top-cycle' : null,
    slotActive(state) ? 'clicky-glow-channel-cycle' : null,
  ].filter(Boolean);
  const rep = v => anims.map(() => v).join(', ');
  // The recess top tracks the cap's DESCENT (0 → bottom → back), not the shadow's
  // flush-gated window: it must stay glued to the sinking cap edge the whole way,
  // and the flush offsets collapse to a zero-width spike when press-depth ===
  // wall-h (the default), which would flash the recess for one frame only.
  const cavityKf = cavityReveals ? `

/* Cavity-reveal, click-mode full press cycle (issue #96) — the recess top edge
   tracks the descending cap down and back, so the dark recess fills the band
   the cap vacates (never bare housing chrome above the sinking cap). */
@keyframes clicky-cavity-top-cycle {
  0% { top: var(--wall-h); }
  ${pct(sh.bottom)} { top: ${CAVITY_PRESSED_TOP}; }
  100% { top: var(--wall-h); }
}` : '';
  const glowKf = slotActive(state) ? `
...
${sh.animates
  ...
  return `${cavityKf}${glowKf}

.btn-cell:has(.clicky-btn.clicky-press)::before {
  animation-name: ${anims.join(', ')};
  animation-duration: ${rep('calc(var(--press-duration) + var(--duration))')};
  animation-timing-function: ${rep('linear')};
  animation-fill-mode: ${rep('forwards')};
}`;
})()}
