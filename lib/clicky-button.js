 ...
 * @property {'wrap'|'nowrap'|'wrap-reverse'} gridWrap
 ...
 *   Face/cap only — the chrome housing radius is derived (concentric).
 * @property {number}  chromeRadiusOffset  — px added to the concentric
 *   housing radius (cap radius + frame width); 0 = pure concentric ring
 ...
 *   fixed angle desyncs from non-square housings or a changed chromeRadiusOffset.
 ...
 * @property {number}  taper               — % of container width — per-side inward inset of the cap top (mechanical-keycap trapezoid); 0 = straight walls
 * @property {number}  dish                — % — concavity depth of the face top surface; 0 = flat
...
const defaultClickyConfig = {
  ...
  // per-corner values. Face/cap only — the chrome housing radius (derived)
  // stays a separate, uniform family (see buildVarMap).
  radiusCorners:   null,
  chromeRadiusOffset: 0,
  ...
  // Mechanical-keycap sculpt (2.1.0). Both default 0 → today's straight-walled
  // flat-top button, byte-identical (D3). `taper` narrows the cap toward the
  // top (the wall/skirt slopes inward going up, wider base → narrower face —
  // the keyboard-keycap trapezoid); `dish` concaves the face/dome top surface
  // (a soft bowl where a fingertip rests). Both are % of the container width.
  taper:            0,        // % of container width — per-side inward slope of the cap top
  dish:             0,        // % — concavity depth of the face top (0 = flat)
...
// ── Tuning constants ───────────────────────────────────────────
// Bare literals that encode a design/physics decision, named + single-sourced
// so paired sites can't silently desync (issue #91 audit). Where a value is
// physics-locked, the invariant is stated so a future edit can't quietly
// violate it.

// Hover-lift transform transition. Deliberately duration-INDEPENDENT: a hover
// affordance shouldn't slow down when the user cranks `duration`/`speedFactor`.
// Referenced by both `.btn-wall` and sharedFaceCssProps so the wall and cap
// stay glued during hover-in/out (the two must be byte-identical or the layers
// desync). NOTE: a hover→pointerdown within this window still shows a snap —
// the transition→animation handoff has no continuity (issue #94, deferred to
// the verification phase).
const HOVER_TRANSFORM_TRANSITION = '100ms ease-out';

// — Physics-locked lighting ratios (changing these past their stated bound
//   breaks the dimensional read, not just the aesthetic; see issue #91 review).

// Frame-bevel shadow alpha as a fraction of the highlight alpha. INVARIANT:
// must stay < 1.0 — at ≥1.0 the shadow side reads brighter than the highlight
// and the light source visually inverts. (~0.6–0.8 reads as ambient fill.)
const FRAME_BEVEL_SHADOW_RATIO = 0.7;

// Secondary (L2) recess band, as fractions of the primary inset's offset/blur.
// INVARIANT: blur fraction must exceed offset fraction, so L2 reads as a
// softer, more diffuse bounce than L1's crisper primary occlusion.
const RECESS_BOT_OFFSET_FRAC = 0.45;
const RECESS_BOT_BLUR_FRAC   = 0.65;

// Tight contact/AO shadow, as multiples of wall height. INVARIANTS: blur ≈
// 2× offset (a recognizable contact-shadow convention), and both must stay
// ≪ the ambient shadow's own reach (AMBIENT_*_MULT below) or it stops reading
// as "seated contact" and just duplicates the ambient layer.
const CONTACT_OFFSET_MULT = 0.25;
const CONTACT_BLUR_MULT   = 0.5;

// Frameless-mode cavity darken %. In frameless mode this is the ONLY cue that
// the cavity is a recess rather than a same-color continuation of the wall.
// INVARIANT: must stay > 0 (darker than the wall) and well short of ~90
// (near-black reads as a punched-out void, not a shaded surface).
const FRAMELESS_CAVITY_DARKEN_PCT = 45;

// Rim highlight blur as a fraction of rim height. INVARIANT: must stay < 1×
// or the thin bright edge smears into a diffuse glow and loses its "edge" read.
const RIM_BLUR_FRAC = 0.5;

// Press-motion duration scaling. distScale ≈ 1 at the library's DEFAULT
// geometry: REFERENCE_TRAVEL_PX is the total key travel (pressDepth+inset) at
// 88px container / 16%/16%/8% ratios. This makes duration scale with travel
// distance (constant-velocity feel) rather than staying fixed while the key
// moves farther/less — real physics, not a free tuning knob.
const REFERENCE_TRAVEL_PX = 21;
const DIST_SCALE_MIN = 0.5;
const DIST_SCALE_MAX = 3;

// Travelling-halo geometry, as multiples of wall height. The 1.5/0.5 SPLIT is
// free aesthetic (an emissive halo, not a directional cast shadow), but their
// SUM is load-bearing: it is the halo's max reach, which --slot-breathing-room
// reserves as slot padding (derived from these below) — if they drift apart
// the halo clips the neighbouring slot.
const GLOW_BLUR_MULT   = 1.5;
const GLOW_SPREAD_MULT  = 0.5;

// — Free-aesthetic constants (extracted for single-sourcing/hygiene only).

// Frame-width caps as fractions of container W/H (the engine already bounds
// frameWidth so an absolute-px frame can't swallow a small button).
const FRAME_MAX_W_FRAC = 0.25;
const FRAME_MAX_H_FRAC = 0.40;

// Auto font-size (when fontSizeRatio is 0), as fractions of container W/H.
// Single source for BOTH the baked-px path and the live cqi/cqb fallback —
// they must stay mirrored (0.08 ↔ 8cqi, 0.22 ↔ 22cqb) or the two rendering
// paths disagree.
const AUTO_FONT_W_FRAC = 0.08;
const AUTO_FONT_H_FRAC = 0.22;

// Focus "tint" fill: fraction of the focus color mixed over the face. Single
// source for BOTH the JS pre-16.2 fallback mix and the CSS color-mix() % —
// they must agree or the fallback mistints.
const FOCUS_TINT_FRAC = 0.18;

// Focus-glow blur as a multiple of focusSize.
const FOCUS_GLOW_BLUR_MULT = 3;

// Speed factor is a base-10 exponent (each ±1 is a 10× duration change).
const SPEED_DECADE_BASE = 10;

// Per-property animation-timing fraction bounds (fraction of the press cycle):
// keep delays < half-cycle, and the animatable window above a floor so the
// keyframe pair never degenerates.
const CYCLE_FRAC_MAX = 0.49;
const COLOR_ANIM_FRAC_MIN = 0.01;
...
function resolveShadowTiming(state) {
  ...
  const manual    = Math.min(CYCLE_FRAC_MAX, Math.max(0, (state.shadowAnimDelay || 0) / 100));
...
function housingShadowLayers(state, pressed, unit) {
  // unit: 'px' (baked, no-cq fallback) | 'cv' (live calc against --wall-h, so
  // the ambient/contact reach scales with the button — issue #96 slice 4). The
  // length helpers EMBED the unit so the px path stays byte-identical (callers
  // interpolate `${l.y}` with no trailing 'px').
  unit = unit === 'cv' ? 'cv' : 'px';
  const wallH_px = Math.max(1, Math.round(state.containerHeight * state.wallHRatio / 100));
  const keep = pressed ? 1 - (state.ambientPressReduction / 100) : 1;
  const L = mult => unit === 'cv'
    ? `calc(var(--wall-h) * ${(mult * keep).toFixed(4)})`
    : `${Math.round(wallH_px * mult * keep)}px`;
  const Lfloor = (mult, floorPx) => unit === 'cv'
    ? `max(${floorPx}px, calc(var(--wall-h) * ${(mult * keep).toFixed(4)}))`
    : `${Math.max(floorPx, Math.round(wallH_px * mult * keep))}px`;
  const ambient = {
    y: L(state.ambientYMult),
    blur: L(state.ambientBlurMult),
  ...
  const contact = state.contactIntensity > 0
    ? {
        y: Lfloor(CONTACT_OFFSET_MULT, 1),
        blur: Lfloor(CONTACT_BLUR_MULT, 2),
...
function slotGlowLayers(state, pressed, unit) {
  unit = unit === 'cv' ? 'cv' : 'px';
  const wallH_px = Math.max(1, Math.round(state.containerHeight * state.wallHRatio / 100));
  const keep = pressed ? 1 - (state.ambientPressReduction / 100) : 1;
  const L = mult => unit === 'cv'
  ...
  return { blur: L(GLOW_BLUR_MULT), spread: L(GLOW_SPREAD_MULT), color: hexToRgba(state.glowColor, alpha) };
}

function buildGlowShadow(state, pressed, unit) {
  const { blur, spread, color } = slotGlowLayers(state, pressed, unit);
...
function buildHousingDropShadow(state, pressed, unit) {
  const { ambient, contact } = housingShadowLayers(state, pressed, unit);
  const fn = l => `drop-shadow(0 ${l.y} ${l.blur} ${l.color})`;
  return fn(ambient) + (contact ? ` ${fn(contact)}` : '');
}

function buildHousingShadow(state, pressed, unit) {
  ...
  // rendering both read. Length values already carry their unit (px or cv).
  const { ambient, contact } = housingShadowLayers(state, pressed, unit);
  const contactCss = contact
    ? `, 0 ${contact.y} ${contact.blur} 0 ${contact.color}`
    : '';

  return `0 ${ambient.y} ${ambient.blur} 0 ${ambient.color}${contactCss}`;
}
...
// could drift apart.
// Length emitters shared by the face-shadow / rim builders. `unit`:
//   'px'  — generation-time px (no-cq fallback, D5),
//   'cqb' — live % of the query container's block size (legacy live path),
//   'cv'  — live calc() against the `--container-height` var (issue #96): equals
//           the px value when the var is base-px (non-responsive configs) and
//           scales when the geometry override redefines it. `cv` is what the
//           cqSupportsBlock uses now that `.btn-cell` is no longer a container
//           (a bare cqb would resolve against `.btn-scale` = housing, too big).
// `n` is a percentage of container height. negLen keeps a leading-minus offset
// valid ('-calc(...)' is illegal CSS; 'calc(... * -k)' is not).
function shadowLen(unit, chPx, n) {
  if (unit === 'px') return `${Math.round(n * chPx / 100)}px`;
  if (unit === 'cv') return `calc(var(--container-height) * ${(n / 100).toFixed(4)})`;
  return `${n}cqb`;
}
function shadowNegLen(unit, chPx, n) {
  if (unit === 'px') return `-${Math.round(n * chPx / 100)}px`;
  if (unit === 'cv') return `calc(var(--container-height) * ${(-n / 100).toFixed(4)})`;
  return `-${n}cqb`;
}
...
function buildRimLayer(state, unit) {
  unit = unit === 'px' ? 'px' : (unit === 'cv' ? 'cv' : 'cqb');
  const px = n => Math.round(n * state.containerHeight / 100);
  const len = n => shadowLen(unit, state.containerHeight, n);
  ...
  const rimBlur = Math.max(2, Math.round(state.rimHeightRatio * RIM_BLUR_FRAC));
  const rimBlurLen = unit === 'px' ? `${Math.max(2, px(rimBlur))}px` : len(rimBlur);
  return `inset 0 ${len(state.rimHeightRatio)} ${rimBlurLen} 0 ${hlColor}`;
}

function buildFaceShadow(state, pressed, unit) {
  // unit: see shadowLen above — 'cqb' (default) | 'px' | 'cv'.
  unit = unit === 'px' ? 'px' : (unit === 'cv' ? 'cv' : 'cqb');
  const px = n => Math.round(n * state.containerHeight / 100);
  const len = n => shadowLen(unit, state.containerHeight, n);
  const negLen = n => shadowNegLen(unit, state.containerHeight, n);
  ...
  const insetYBot = Math.round(insetY * RECESS_BOT_OFFSET_FRAC);
  const insetBBot = Math.round(insetB * RECESS_BOT_BLUR_FRAC);
  const L2 = pressed
    ? `inset 0 ${negLen(insetYBot)} ${len(insetBBot)} 0 ${botColor}`
...
function buildVarMap(state) {
  ...
  const fwMaxByWidth  = Math.floor(state.containerWidth  * FRAME_MAX_W_FRAC);
  const fwMaxByHeight = Math.floor(state.containerHeight * FRAME_MAX_H_FRAC);
  ...
  const distScale = Math.max(DIST_SCALE_MIN, Math.min(DIST_SCALE_MAX, (pressDepth_px + insetY_px) / REFERENCE_TRAVEL_PX));
  ...
  // Even ring around the CHANNEL (owner directive): the housing reserves fw of
  // chrome around the OPENING the key sinks into, not around the proud resting
  // key. The channel top sits wallH below the cap top; centring the ring on the
  // key (H0 = containerHeight + 2fw, cellTop = fw) therefore pushed the channel
  // down by wallH and read as excess top padding. This formula puts the channel
  // fw from every housing edge:
  //   H0 = max(fw, wallH) + faceH + fw      (faceH = containerHeight - wallH)
  //   cellTop = max(0, fw - wallH)           (restingChromeAbove below)
  // At flush the face lands inside an even fw ring; at rest the proud key stands
  // above the plate and covers the top chrome (correct — a keycap sits above its
  // slot). radius-bot stays R + fw: with the channel inset fw all around, the
  // housing is now concentric with the CHANNEL. A wall deeper than the frame
  // rises higher than the ring, so its top reads wallH — the honest projection.
  // Reduces to the historical containerHeight + fw for the default (wallH === fw)
  // and frameless (fw === 0); it only drifted in between (issue #90 revisited).
  ...
  // it. `chromeRadiusOffset` (px, default 0) nudges this derived radius up or
  // down for configs that want the housing a touch rounder/tighter than pure
  // concentric — the generator's "Chrome Radius ± px offset" control.)
  const chromeRadius_px = fw > 0
    ? Math.max(0, Math.min(radius_px + fw + state.chromeRadiusOffset, Math.floor(housingW / 2), Math.floor(housingH / 2)))
    : radius_px;
  ...
  return {
    ...
    ...(slotActive(state) ? {
      '--slot-breathing-room': `${Math.round(wallH_px * (GLOW_BLUR_MULT + GLOW_SPREAD_MULT))}px`,
    } : {}),

    '--housing-width':  `${housingW}px`,
    '--housing-height': `${housingH}px`,

    // Always-px authored footprint (issue #96) — sizes the `.btn-scale`
    // responsive wrapper and is the no-cq fallback. Never expressed in cq
    // units (it is the reference the cq layer scales relative to).
    '--housing-width-base':  `${housingW}px`,
    '--housing-height-base': `${housingH}px`,
    ...
    '--font-size':       state.fontSizeRatio > 0
      ...
      : `min(${Math.round(state.containerWidth * AUTO_FONT_W_FRAC)}px, ${Math.round(state.containerHeight * AUTO_FONT_H_FRAC)}px)`,
    ...
    // Mechanical-keycap sculpt (2.1.0). --taper: per-side inward inset of the
    // cap TOP (px of container width), so the face narrows and the wall flares
    // to a trapezoid. --dish: concavity depth 0..1 for the face's bowl gradient.
    '--taper':          `${Math.round(state.containerWidth * state.taper / 100)}px`,
    '--dish':           `${(state.dish / 100).toFixed(3)}`,
    ...
    '--cavity-wall-color': state.useCavityWallColor ? state.cavityWallColor
      ...
      : darkenColor(faceFamily.buttonWallColor, FRAMELESS_CAVITY_DARKEN_PCT),
    ...
    '--frame-bevel-alpha-shadow': `${(state.frameBevelAlpha / 100 * FRAME_BEVEL_SHADOW_RATIO).toFixed(3)}`,
    ...
    '--duration':       `${Math.round(state.duration * Math.pow(SPEED_DECADE_BASE, -state.speedFactor) * distScale)}ms`,
    '--press-duration': `${Math.round(state.pressDuration * Math.pow(SPEED_DECADE_BASE, -state.speedFactor) * distScale)}ms`,
    ...
    ...(function () {
      ...
      const manualFrac = Math.min(CYCLE_FRAC_MAX, Math.max(0, state.shadowAnimDelay / 100));
      const effFlush = Math.min(1, flushFrac + manualFrac * (1 - flushFrac));
      const colorFrac = Math.min(CYCLE_FRAC_MAX, state.colorAnimDelay / 100);
      return {
        ...
        '--color-anim-frac':        Math.max(COLOR_ANIM_FRAC_MIN, 1 - 2 * colorFrac).toFixed(3),
      };
    })(),
...
// Live scale-space geometry override (issue #96). `.btn-scale` is the SOLE
// container-query container (the nested `.btn-cell` is no longer one — see
// buildCss's cqSupportsBlock), so every var here resolves against `.btn-scale`
// and the box reflows to whatever size/ratio the consumer gives it. Handles
// EVERY form (v2.0.1): the common path, per-corner radius, skew (via the
// baked-tan base-dim recovery below), and segmented housings (per-segment
// container width) — each verified reflowing in real WebKit. Returns the
// override lines (indented).
function buildGeometryCqLines(state) {
  const skew = resolveSkew(state);

  const cw = state.containerWidth, ch = state.containerHeight;
  const fw = state.frameEnabled
    ? Math.min(state.frameWidth, Math.floor(cw * FRAME_MAX_W_FRAC), Math.floor(ch * FRAME_MAX_H_FRAC))
    : 0;
  const num = n => +n.toFixed(4);
  const off = state.chromeRadiusOffset || 0;

  // Segmented housing (issue #36): N segments share one housing, so the base
  // housing width holds N container widths + the N−1 dividers + the frame. Each
  // segment's own container-width is that back-solved and divided by N below.
  const segmented = state.housingLayout === 'segmented';
  const segCount = segmented ? Math.max(1, getLabels(state).length) : 1;
  const dividersTotal = segCount > 1 ? (segCount - 1) * state.segmentDividerWidth : 0;

  // Full housing dims (matching buildVarMap's W0/H0 + skew widen) — used for the
  // cqmin frame balance, and, for skew, for the base-dim derivation below.
  const tanX = skew.active ? Math.abs(Math.tan(skew.x * Math.PI / 180)) : 0;
  const tanY = skew.active ? Math.abs(Math.tan(skew.y * Math.PI / 180)) : 0;
  const W0 = (segmented ? segCount * cw + dividersTotal : cw) + 2 * fw;
  const H0 = ch + 2 * fw;
  const widenX = skew.active ? Math.ceil(H0 * tanX + W0 * tanX * tanY) : 0;
  const widenY = skew.active ? Math.ceil(W0 * tanY) : 0;
  const housingW = W0 + widenX, housingH = H0 + widenY;
  // Frame band proportional to the SMALLER axis (cqmin) so it stays a balanced
  // uniform ring at any aspect ratio instead of fattening on the long side.
  const fwCqmin = fw > 0 ? (fw / Math.min(housingW, housingH) * 100) : 0;

  // container-width/height expressions. Skew widens the housing to reserve room
  // for the sheared parallelogram; .btn-scale is the FULL (widened) housing, so
  // 100cqw/cqh is housingW/housingH. Recover the base W0/H0 from that: the widen
  // algebra collapses to W0 = housingW − housingH·tanX, H0 = housingH − W0·tanY
  // (the tanX·tanY cross term cancels). tan values are baked constants (the skew
  // angle is fixed config, not responsive). --skew-widen-y feeds cellTopExpr.
  const skewBaseLines = skew.active ? [
    `--skew-base-w: calc(100cqw - 100cqh * ${num(tanX)});`,
    `--skew-base-h: calc(100cqh - var(--skew-base-w) * ${num(tanY)});`,
    `--skew-widen-y: calc(var(--skew-base-w) * ${num(tanY)});`,
  ] : [];
  // Base (un-frame) housing width: the skew-recovered W0 when skewed, else the
  // full scale width. For segmented, the per-segment container-width is that
  // minus the frame and dividers, divided by the segment count.
  const baseWexpr = skew.active ? 'var(--skew-base-w)' : '100cqw';
  const baseHexpr = skew.active ? 'var(--skew-base-h)' : '100cqh';
  const cwExpr = segmented
    ? `calc((${baseWexpr} - ${dividersTotal}px - 2 * var(--frame-width)) / ${segCount})`
    : `calc(${baseWexpr} - 2 * var(--frame-width))`;
  const chExpr = `calc(${baseHexpr} - 2 * var(--frame-width))`;

  // Cap radius clamped exactly as the px path's maxRadiusPx: half container
  // width, half faceH (container-height − wall-h), half container height.
  const radiusExpr = `min(calc(var(--container-width) * ${num(state.radiusRatio / 100)}), calc(var(--container-width) / 2), calc((var(--container-height) - var(--wall-h)) / 2), calc(var(--container-height) / 2))`;
  // THE #90 INVARIANT, live: housing radius is a TEXTUAL reference to the cap
  // radius + frame width (both resolve in the same single container), so
  // Rh === R + fw holds at every size by construction. Clamped to half housing.
  const radiusBotExpr = fw > 0
    ? `min(calc(var(--radius) + var(--frame-width) + ${off}px), calc(var(--housing-width) / 2), calc(var(--housing-height) / 2))`
    : `var(--radius)`;

  const lines = [
    `--housing-width: 100cqw;`,
    `--housing-height: 100cqh;`,
    ...skewBaseLines,
    `--frame-width: ${fw > 0 ? `${num(fwCqmin)}cqmin` : '0px'};`,
    `--container-width: ${cwExpr};`,
    `--container-height: ${chExpr};`,
    `--wall-h: calc(var(--container-height) * ${num(state.wallHRatio / 100)});`,
    `--press-translate: calc(var(--container-height) * ${num(state.pressDepthRatio / 100)});`,
    `--toggle-height: calc(var(--press-translate) * ${num(state.toggleHeightRatio / 100)});`,
    `--radius: ${radiusExpr};`,
    `--radius-bot: ${radiusBotExpr};`,
  ];

  // Per-corner cap radius (issue #35), live. Each corner is clamped by the same
  // per-axis family as the uniform --radius; the browser applies the CSS
  // adjacent-corner overlap shrink LIVE, so (unlike the frozen-px path's
  // clampRadiusCorners) no JS-side overlap pass is needed. --radius stays
  // uniform for the housing ring (--radius-bot), which is cap-only per #35.
  if (state.radiusCorners) {
    const c = state.radiusCorners;
    const cornerExpr = ratio => `min(calc(var(--container-width) * ${num(ratio / 100)}), calc(var(--container-width) / 2), calc((var(--container-height) - var(--wall-h)) / 2), calc(var(--container-height) / 2))`;
    lines.push(
      `--radius-tl: ${cornerExpr(c.tl)};`,
      `--radius-tr: ${cornerExpr(c.tr)};`,
      `--radius-br: ${cornerExpr(c.br)};`,
      `--radius-bl: ${cornerExpr(c.bl)};`,
    );
  }

  // Conic frame-bevel stop angles, LIVE (issue #96 slice 3). The frozen-px base
  // bakes these from the authored aspect ratio (computeFrameBevelConicStops),
  // so the highlight/shadow sweep drifts off the rounded corner at any other
  // ratio. Expressed as CSS atan2() over the live housing half-dims + radius,
  // they track the real corner at every ratio. Same formulas as the JS:
  // trStart = atan2(w−r, h), trEnd = atan2(w, h−r); bl = tr + 180°. Only when
  // the conic bevel is active (else the base vars aren't even emitted).
  if (state.frameBevelConic && fw > 0) {
    const w = 'calc(var(--housing-width) / 2)';
    const h = 'calc(var(--housing-height) / 2)';
    lines.push(
      `--frame-bevel-conic-tr-start: atan2(calc(${w} - var(--radius-bot)), ${h});`,
      `--frame-bevel-conic-tr-end: atan2(${w}, calc(${h} - var(--radius-bot)));`,
      `--frame-bevel-conic-bl-start: calc(var(--frame-bevel-conic-tr-start) + 180deg);`,
      `--frame-bevel-conic-bl-end: calc(var(--frame-bevel-conic-tr-end) + 180deg);`,
    );
  }

  // Ambient/contact drop-shadow + travelling glow, scaled off the live --wall-h
  // (issue #96 slice 4) so the shadow reach tracks the button size instead of
  // staying a frozen px blob that reads oversized once the button shrinks.
  lines.push(
    `--housing-shadow: ${buildHousingShadow(state, false, 'cv')};`,
    `--housing-shadow-pressed: ${buildHousingShadow(state, true, 'cv')};`,
  );
  if (!state.frameEnabled) {
    lines.push(
      `--housing-drop-shadow: ${buildHousingDropShadow(state, false, 'cv')};`,
      `--housing-drop-shadow-pressed: ${buildHousingDropShadow(state, true, 'cv')};`,
    );
  }
  if (slotActive(state)) {
    // Halo reach and the slot padding that reserves room for it must both track
    // --wall-h or the glow clips the neighbouring slot at other sizes.
    lines.push(
      `--slot-breathing-room: calc(var(--wall-h) * ${num(GLOW_BLUR_MULT + GLOW_SPREAD_MULT)});`,
      `--glow-shadow: ${buildGlowShadow(state, false, 'cv')};`,
      `--glow-shadow-pressed: ${buildGlowShadow(state, true, 'cv')};`,
    );
  }
  return lines.map(l => '\n      ' + l).join('');
}
...
function buildCss(state, scopeSelector) {
  ...
  // All cq-relative values now use the 'cv' path (calc against --container-*)
  // rather than raw cqi/cqb, because `.btn-cell` is no longer a container
  // (issue #96): a bare cqb would resolve against `.btn-scale` (= housing,
  // 2·frame too big). When --container-* are base px (non-common configs with
  // no geometry override) these equal today's frozen output; when the geometry
  // override redefines them, they scale live.
  const fontSizeCq = state.fontSizeRatio > 0
    ? `calc(var(--container-width) * ${(state.fontSizeRatio / 100).toFixed(4)})`
    : `min(calc(var(--container-width) * ${AUTO_FONT_W_FRAC}), calc(var(--container-height) * ${AUTO_FONT_H_FRAC}))`;
  const faceShadowRestingCq = buildFaceShadow(state, false, 'cv');
  ...
  const faceShadowPressedCq = buildFaceShadow(state, resolveShadowTiming(state).animates, 'cv');
  // Independent rim light (issue #74) — same cv pairing as --face-shadow-*
  // above, gated on rimIndependent so the override line only appears when the
  // var itself exists (D3). Attached to the preceding `;` on the SAME line — a
  // ternary emitting '' on its own source line still emits a stray newline even
  // when the branch taken is '' (see the whitespace trap this file has hit).
  const rimShadowCqLine = state.rimIndependent
    ? `\n      --rim-shadow: ${buildRimLayer(state, 'cv')};`
    : '';
  // Live scale-space box geometry (slice 2) — '' on the gated-out paths.
  const geometryCqLines = buildGeometryCqLines(state);
  const cqSupportsBlock = `@supports (width: 1cqi) {
    ${scopeSelector} {${geometryCqLines}
...
.btn-wall {
  ...
  top: ${topExpr};
  height: ${heightExpr};
  border-radius: ${wallRadiusExpr};${state.taper > 0 ? `
  /* Taper (2.1.0): flare the skirt from the narrow face top (inset --taper per
     side) out to the full base — the keyboard-keycap trapezoid. */
  ...
  pointer-events: none;
  transition: transform ${HOVER_TRANSFORM_TRANSITION};
}
...
function buildGridCss(state) {
  ...
  // Cavity top is pinned STATICALLY at --wall-h — the flush line (issue #96).
  // This is the physical keycap model: the cap sits proud by its wall height in
  // a housing whose top surface is the plate. As the key presses DOWN, the band
  // it vacates above the cap reveals, in order:
  //   • pre-flush  (travel < wall-h): the HOUSING SURFACE — the vacated band is
  //     entirely ABOVE the cavity top (--wall-h), so the chrome plate shows,
  //     and the cavity stays hidden behind the still-proud cap.
  //   • post-flush (travel > wall-h): the CHANNEL — the band now extends past
  //     --wall-h into the cavity, uncovering the housing-slot wall
  //     (--cavity-wall-color, chrome by default) beneath the sunk cap.
  // The cavity NEVER moves: the cap uncovers a fixed hole, so there is no `top`
  // transition/keyframe and thus no channel bounce (owner report). Static at
  // --wall-h works for every press depth — shallow (cap never reaches it → its
  // own footprint covers the gap, nothing revealed) and deep (channel fills the
  // whole post-flush band) alike.
  const cavityStaticTop = 'var(--wall-h)';
  // Only the lit glow channel (slot-active) still transitions/animates — a
  // light transfer, not a geometry move. The cavity `top` never transitions.
  const _cavGlowRel = 'var(--shadow-release-dur) var(--shadow-ease-release)';
  const _cavGlowPrs = 'var(--shadow-press-dur) var(--shadow-ease-press) var(--shadow-press-delay)';
  const cavityBaseTransition = slotActive(state) ? `background-image ${_cavGlowRel}` : '';
  const cavityPressOverride = slotActive(state) ? `
...
.btn-cell:has(.clicky-toggle input:checked)::before {
  ...
  transition: background-image ${_cavGlowPrs};
}` : '';
...
`;
  ...
  const cellInsetExpr = skew.active
    ...
    : 'var(--frame-width)';
  // The cell is inset by max(0, fw - wallH) at the top so the CHANNEL it holds
  // (at cellTop + wallH) sits fw from the housing top — an even fw ring around
  // the channel, matching the sides and bottom (owner directive). The proud key
  // stands wallH above that, covering the top chrome at rest (a keycap sits
  // above its slot). Floored at 0 so a proud key never clips the housing.
  // Pairs with H0 = max(fw, wallH) + faceH + fw in buildVarMap.
  // A proud cap sits flush to the top (cellTop → 0): the top chrome band is
  // fully obscured by the cap (owner directive). The corner-tangency "pinch"
  // that this used to cause — the cap's rounded corner meeting the housing's —
  // is NOT solved here with a top floor (that just reads as top padding);
  // instead the HOUSING's top corners are shrunk to hug the cap (see
  // housingRadiusTop below), so the cap corner never has a bigger housing corner
  // to peek past.
  ...
  const cellTopExpr = skew.active
    ...
    : restingChromeAbove;
  // Housing top-corner radius hugs the (possibly proud) cap. It is the full
  // chrome radius (--radius-bot) minus how much of the frame the proud cap has
  // swallowed at the top, which is exactly min(fw, wallH) = fw - cellTop:
  //   • flat cap  (wallH 0 → cellTop fw): subtract 0  → --radius-bot (uniform
  //     fw ring — renders identically to the pre-fix housing; the expression text
  //     differs, so snapshots update, but the resolved corner is unchanged).
  //   • proud cap (wallH ≥ fw → cellTop 0): subtract fw → --radius (== R), so the
  //     housing corner sits directly on the cap corner — no gap/pinch at the top.
  // Bottom corners stay --radius-bot (the cap IS inset fw there → concentric).
  const housingRadiusTop = 'max(0px, calc(var(--radius-bot) - min(var(--frame-width), var(--wall-h))))';
  const housingBorderRadius = `${housingRadiusTop} ${housingRadiusTop} var(--radius-bot) var(--radius-bot)`;
  // The cell is pinned by top + bottom (not a fixed height), so the BOTTOM ring
  // is always exactly frame-width — independent of cellTop. When the proud key
  // lifts cellTop above 0 (was fw), a fixed container-height cell dumped the
  // freed frame to the bottom, doubling it (owner: "bottom too thick"). Pinning
  // the bottom instead keeps the channel bottom ring at fw for every wall depth;
  // the cap (height: 100% - wall-h of the cell) simply fills the space between.
  // Reduces to the old container-height cell whenever cellTop === fw (flat/shallow).
  const cellBottomExpr = skew.active
    ? 'calc(var(--frame-width) + var(--skew-widen-y, 0px) / 2)'
    : 'var(--frame-width)';
...
/* Responsive sizing boundary (issue #96). Defaults to the authored footprint
   so a standalone or gridded button looks exactly as before; a consumer that
   gives it a different size (flex stretch, width/height, aspect-ratio) makes
   every box measure below reflow against it via container-query units. It is a
   container-type: size box, so it never derives its own size from its contents
   — the authored-px default (or the consumer's override) is what it queries.
   No overflow/isolation here: containment must not clip the glow halo. */
.btn-scale {
  container-type: size;
  width: var(--housing-width-base);
  height: var(--housing-height-base);
  max-width: 100%;
}
${slotActive(state) ? `
.btn-slot {
...
.btn-housing {
  ...
  border-radius: ${housingBorderRadius};
  overflow: hidden;
...
.btn-cell {
  ...
  left: ${cellInsetExpr};
  right: ${cellInsetExpr};
  bottom: ${cellBottomExpr};
  /* NOT a container-type here (issue #96): .btn-scale is the single cq
     container, so every geometry var resolves in one space and the concentric
     ring invariant (--radius-bot === --radius + --frame-width) holds textually.
     A nested container would resolve shared vars against two different boxes
     (verified in real WebKit). position:absolute still makes this the
     containing block for the cap/wall, which is all that is needed here. */
  overflow: hidden;
...
.btn-cell::before {
  content: '';
  position: absolute;
  /* STATIC recess pinned at the flush line (top: --wall-h) — it does NOT
     animate. Hidden at rest behind the proud cap; as the key sinks the cap
     uncovers the housing surface first (pre-flush, above this edge) and then
     this channel (post-flush, below it). See cavityStaticTop.
  ...
  background-blend-mode: multiply;
  pointer-events: none;${cavityBaseTransition ? `
  transition: ${cavityBaseTransition};` : ''}
}${cavityPressOverride}${segmentedCss}${!slotActive(state) ? '' : `
...
function sharedFaceCssProps(state, indent) {
  const i = indent || '  ';
  // Taper (2.1.0): the cap TOP (face) is inset an extra --taper per side so it
  // narrows relative to the base; the wall flares out to it (buildButtonWallCss
  // adds the trapezoid clip). taper 0 ⇒ faceInsetDecl, byte-identical (D3).
  const faceLR = state.taper > 0
    ? (state.faceTolerance > 0 ? 'calc(var(--face-tolerance) + var(--taper))' : 'var(--taper)')
    : faceInsetDecl(state);
  return [
    `${i}position: absolute;`,
    `${i}top: 0;`,
    ...
    // pre-#73 output).
    // Dish (2.1.0) — a soft concave bowl on the face top: the near (upper) rim
    // curves away from the top light so it shades, the far (lower) rim catches
    // it. Composes with an inline specular hotspot when that isn't on the
    // independent `.btn-face::after` layer. dish 0 AND no inline specular ⇒ no
    // background-image at all — byte-identical to pre-2.1.0 output (D3). The
    // specular-only branch is also byte-identical to the pre-dish emission.
    ...((state.dish > 0) || (state.specularAlpha > 0 && !state.specularIndependent) ? (() => {
      const layers = [], blends = [];
      if (state.dish > 0) {
        layers.push(
          `radial-gradient(ellipse 150% 100% at 50% -12%, rgb(0 0 0 / calc(var(--dish) * 0.55)), transparent 52%)`,
          `radial-gradient(ellipse 150% 100% at 50% 112%, rgb(255 255 255 / calc(var(--dish) * 0.5)), transparent 52%)`,
        );
        blends.push('normal', 'normal');
      }
      if (state.specularAlpha > 0 && !state.specularIndependent) {
        layers.push(`radial-gradient(circle at var(--light-x) var(--light-y), rgba(255, 255, 255, var(--specular-alpha)) 0%, transparent var(--specular-size))`);
        blends.push('soft-light');
      }
      return [
        `${i}background-image: ${layers.join(', ')};`,
        `${i}background-blend-mode: ${blends.join(', ')};`,
      ];
    })() : []),
    ...
    `${i}transition:`,
    `${i}  transform ${HOVER_TRANSFORM_TRANSITION},`,
...
function buildFocusVisibleCss(state, selector) {
  ...
  switch (state.focusStyle) {
    case 'tint': {
      ...
      if (isHexColor(c) && isHexColor(state.faceColor)) {
        ...
        const mixCh = (a, b) => Math.round(FOCUS_TINT_FRAC * parseInt(a, 16) + (1 - FOCUS_TINT_FRAC) * parseInt(b, 16));
      ...
      return `${selector} {
  ${fallbackLine}background-color: color-mix(in srgb, ${c} ${Math.round(FOCUS_TINT_FRAC * 100)}%, var(--face-color));
  }`;
    }
    case 'glow':
      return `${selector} {
  box-shadow:
    ...
    0 0 0 ${sz}px ${c}66,
    0 0 ${sz * FOCUS_GLOW_BLUR_MULT}px ${sz}px ${c}55;
}`;
    case 'outline':
      return `${selector} {
...
.clicky-btn.clicky-press .btn-wall {
  ...
  // Momentary-click (.clicky-press) lit-glow-channel handling (issue #53b —
  // slot-active only). The recess `top` is STATIC now (see cavityStaticTop), so
  // there is no cavity-top keyframe — the cap uncovers a fixed recess, no
  // channel animation. Nothing emitted with no glow (byte-identical).
...
function buildSingleButtonHtml(state, label, extraClass, extraAttrs) {
  ...
  // `.btn-scale` (issue #96) — the UNCONDITIONAL responsive-sizing boundary
  // (container-type: size). Wraps the slot/housing so all box geometry can
  // resolve against it via cq units and reflow to any container size/ratio.
  // Must always be emitted (an @supports cq override with no container
  // ancestor resolves against nothing — see buildGridCss).
  const slotOpen = '<div class="btn-scale">' + (slotActive(state) ? '<div class="btn-slot">' : '');
  ...
  const slotClose = (slotActive(state) ? '</div>' : '') + '</div>';
  if (state.mode === 'click') {
...
function buildSegmentedHousingHtml(state, opts, groupLabel) {
  ...
  // `.btn-scale` (issue #96) — unconditional responsive-sizing boundary
  // wrapping the shared segmented housing (one per shared housing).
  if (!slotActive(state)) return `<div class="btn-scale">${housingHtml}</div>`;
  ...
  return `<div class="btn-scale"><div class="btn-slot">${housingHtml}\n  ${glowChildren}\n</div></div>`;
}
...
function validateClickyConfig(config) {
  ...
  if (housingLayout === 'segmented') {
    const gw = effective('gridWrap', 'string');
    if (gw === 'wrap' || gw === 'wrap-reverse') {
      issues.push(`housingLayout "segmented" cannot be combined with gridWrap "${gw}" — a wrapped segment strip is meaningless`);
    }
...
function buildClickyHtml({ label, tag = 'button', attrs = {}, extraClass = '', config: userConfig = {} }) {
  ...
  // `.btn-scale` (issue #96) — unconditional responsive-sizing boundary; see
  // buildSingleButtonHtml for the rationale.
  const slotOpen = '<div class="btn-scale">' + (slotActive(config) ? '<div class="btn-slot">' : '');
  const glowHtml = slotActive(config) ? '<div class="btn-glow"></div>' : '';
  const slotClose = (slotActive(config) ? '</div>' : '') + '</div>';
