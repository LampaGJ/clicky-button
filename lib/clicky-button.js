...
function buildFaceShadow(state, pressed) {
  ...
  // The button wall (moving side of the keycap) is drawn as the face's
  // ::after pseudo-element — see buildButtonWallCss — not here. As a child of
  // the face it tracks every transform (hover, click cycle, toggle) for free.
...
/* The button wall (moving side of the keycap) is drawn as a bottom background
   band on .btn-face itself — see sharedFaceCssProps — not as a separate element.
   Keeping cap + wall in one continuous rounded shape is what makes the wall
   follow the cap's silhouette without leaking the cavity at rounded corners.
   This block only styles the label wrapper. */
function buildButtonWallCss() {
  return `/* Button label — wraps the face text so it stays clipped to the cap
   region and never overflows the rounded shape. */
.btn-label {
  display: block;
  max-width: 100%;
  overflow: hidden;
...
.btn-cell::before {
  ...
  /* Cavity wall — the static inside of the housing slot, revealed above the
     ...
     moving button-side wall is the bottom band of the .btn-face background,
     using the same gradient structure driven by its own button-wall vars.) */
...
function sharedFaceCssProps(indent) {
  ...
  return [
    ...
    `${i}left: 0;`,
    `${i}right: 0;`,
    `${i}height: calc(100% + var(--hover-lift));`,
    `${i}display: flex;`,
    `${i}align-items: center;`,
    `${i}justify-content: center;`,
    `${i}border-radius: var(--radius);`,
    // Face spans the WHOLE key (cap + wall) as one continuous rounded shape so
    // the wall follows the cap's rounded silhouette exactly — no chrome wedge at
    // the corners, at any radius. The cap is the face color; the button wall is
    // a bottom background band (wall color × L-R darken vignette) sized
    // (wall-h + hover-lift) tall, so on hover the band stays flush with the cell
    // bottom instead of exposing a strip of cavity. The label is centered in the
    // cap region via the matching padding-bottom below.
    `${i}background-color: var(--face-color);`,
    `${i}background-image:`,
    `${i}  linear-gradient(`,
    `${i}    to right,`,
    `${i}    rgba(0, 0, 0, var(--button-wall-shadow-alpha))                                                0%,`,
    `${i}    rgba(0, 0, 0, calc(var(--button-wall-shadow-alpha) * var(--button-wall-shadow-edge-ratio))) var(--button-wall-gradient-lo),`,
    `${i}    rgba(0, 0, 0, calc(var(--button-wall-shadow-alpha) * var(--button-wall-shadow-edge-ratio))) var(--button-wall-gradient-hi),`,
    `${i}    rgba(0, 0, 0, var(--button-wall-shadow-alpha))                                              100%),`,
    `${i}  linear-gradient(var(--button-wall-color), var(--button-wall-color));`,
    `${i}background-repeat: no-repeat;`,
    `${i}background-position: bottom;`,
    `${i}background-size: 100% calc(var(--wall-h) + var(--hover-lift));`,
    `${i}background-blend-mode: multiply, normal;`,
    ...
    `${i}overflow: hidden;`,
    `${i}padding-top: 0;`,
    // Reserve the wall band at the bottom so the flex-centered label sits in the
    // cap region, not over the wall.
    `${i}padding-bottom: calc(var(--wall-h) + var(--hover-lift));`,
...
function buildSingleButtonHtml(state, label, extraClass) {
  ...
  if (state.mode === 'click') {
    return `<div class="btn-housing"><div class="btn-cell"><button class="clicky-btn${labelClass}${extra}" type="button"><span class="btn-face"><span class="btn-label">${txt}</span></span></button></div></div>`;
  }
  return `<div class="btn-housing"><div class="btn-cell"><label class="clicky-toggle${labelClass}${extra}">
  <input type="checkbox">
  <span class="btn-face"><span class="btn-label">${txt}</span></span>
</label></div></div>`;
}
...
function buildClickyHtml({ label, tag = 'button', attrs = {}, extraClass = '', config: userConfig = {} }) {
  ...
  return `<div class="btn-housing"><div class="btn-cell"><${tagName} class="clicky-btn ${cls}"${typeAttr}${attrStr}><span class="btn-face"><span class="btn-label">${txt}</span></span></${tagName}></div></div>`;
}
